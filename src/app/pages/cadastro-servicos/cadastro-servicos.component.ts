...
export class CadastroServicosComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private appService: AppService, private servicoService: CadastroServicosService,private dialog: MatDialog ) {
    appService.getCombo('tipo_servicos')
    .subscribe((data: { query: string, json: Array<Combo> }) => {
        this.comboTipo = data.json;
      });
    appService.getCombo('beneficiados')
    .subscribe((data: { query: string, json: Array<Combo> }) => {
        this.comboBeneficiados = data.json;
      });
    appService.getCombo('servicos')
    .subscribe((data: { query: string, json: Array<Combo> }) => {
  ...
  onChanges(): void {   
    this.servicosForm.get('CUSTO_POR_UNIDADE').valueChanges
      ...
      .subscribe(custo => {
        ...
        this.servicosForm.get('CUSTO_POR_UNIDADE').updateValueAndValidity()
      })
  }
  submitServicos(): void{
    this.submitted = true;
    ...
    let json = Object.assign(servico, this.servicosForm.value);
    json.CUSTO_POR_UNIDADE = json.CUSTO_POR_UNIDADE.replace(',','.');
    json.DESCRICAO = json.DESCRICAO.toUpperCase();
    this.servicoService.cadastroServico(json)
  ...
  selectCliente(item: string): void{
    const obj =  this.comboServicos.find(el => {return el.LABEL === item});
    this.servicosForm.get('ID_SERVICO').setValue(obj.VALOR);
    this.servicoService.getServico(obj.VALOR)
    .subscribe((data: {query:string, json:Array<Servico>}) => {
      if(data.json.length > 0){
  ...
  search = (text$: Observable<string>) =>
    text$.pipe(
      debounceTime(200),
      map(term => term === '' ? []
        : this.comboServicos.filter(v => v.LABEL.toLowerCase().indexOf(term.toLowerCase()) > -1)
